...
def writeLMDB(datasets, lmdb_path, validation):
    ...
    for count in range(numSample):
        ...
        #load bounding box image
        #bb_header = '../dataset/MPI/images/cropped1/'
        bb_path = data[idx]['img_paths'].split('.')[0] + "_0.jpg"
        text_file.write("%s %d\n" % (bb_path,0))
        #print os.path.join(bb_header, bb_path)

        #bb_img = cv2.imread(os.path.join(bb_header,bb_path))
